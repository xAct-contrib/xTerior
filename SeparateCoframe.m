...
(*SeparateCoframe*)
SeparateCoframeAux[form_,list_IndexList:IndexList[]]:=Module[{auxind,formind,lengthformind,compind,deg,tensorhead,mfd,i,j,tensor,tensorind,result},
...
DefTensor[tensor,mfd,PrintAs-> ToString[StringForm["\!\(\*SuperscriptBox[\(\[InvisiblePrefixScriptBase]\), \(\[CircleTimes]\)]\)`1`",PrintAs[Evaluate@Head@form]],StandardForm]];
(*Inherit symmetry from form and respect the antisymmetry in the "component indices"*)
SymmetryGroupOfTensor[tensorhead]^=StrongGenSet[Range[lengthformind+deg],Join[SymmetryGroupOfTensor[Head@form][[2]],Replace[Table[\!\(\*
TagBox[
StyleBox[
RowBox[{"Times", "[", 
RowBox[{
...
NumberMarks->True],
FullForm]\),{j,lengthformind+1,lengthformind+deg-1}]//OutputForm,head_[arg__]:> GenSet[arg],{0,1}]]//.{}->Sequence[]];
];
