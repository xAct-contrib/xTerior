BeginPackage["xAct`FormsToTensors`", {"xAct`xTerior`", "xAct`xCoba`",
	 "xAct`xTensor`", "xAct`xPerm`", "xAct`xCore`"}]

FormsToTensors::usage = " ";

Begin["`Private`"]

(*Threads*)

FormsToTensors[expr_Plus, frame_] :=
	FormsToTensors[#, frame]& /@ expr

FormsToTensors[expr_List, frame_] :=
	FormsToTensors[#, frame]& /@ expr

(*Better idea to implement thread over Wedge? Simple map seems to fail*)

FormsToTensors[expr_Wedge, frame_] :=
	Wedge @@ FormsToTensors[List @@ expr, frame]

FormsToTensors[expr_Times, frame_] :=
	FormsToTensors[#, frame]& /@ expr

FormsToTensors[expr_Diff, frame_] :=
	FormsToTensors[#, frame]& /@ expr

(*Main function*)

FormsToTensors[form_, frame : (xAct`xTerior`Coframe | xAct`xTerior`dx
	)] :=
	Module[{head, tensorhead, deg, i, dummies, indices, ind, basis, out,
		 vb, list1, cycles},
		head = Head @ form;
		deg = xAct`xTerior`Deg @ form;
		...
		If[xTensorQ @ head && deg != 0,
			...
			For[i = 1, i <= deg, i++,
				...
				cycles =
					Prepend[
						...
						If[i != deg,
							-Cycles[
								{
								Length @ SlotsOfTensor @ head + deg - i, 
								Length @ SlotsOfTensor @ head + deg + 1 - i
								}
							]
							,
							Nothing
				...
				ind = DummyIn @ Tangent @ vb;
				dummies = Append[dummies, ind];
				basis = Wedge[basis, frame[DependenciesOfTensor @ head // First][ind]];
			];
			(*Join SG*)
			SymmetryGroupOfTensor[tensorhead] ^= 
				JoinSGS[SymmetryGroupOfTensor[head], StrongGenSet[list1, GenSet @@ cycles]];
			(* Output *)
			out = 
				Wedge[
					(1 / Factorial[deg]) tensorhead @@ Join[
						indices, 
						ChangeIndex[dummies] /. List -> IndexList // Evaluate
						], 
					basis
					];
			Return @ out
